-- ============================================================================
-- AI Pod Staffing - safe resume after ORA-05730 on AUDIT_EVENTS
-- Use this file only after the original installer created 17 empty tables and
-- failed because TIMESTAMP WITH TIME ZONE was unsupported in a blockchain table.
-- Run in SQL Developer with F5 (Run Script), connected as AI_POD_STAFFING.
-- This script never drops or overwrites a table.
-- ============================================================================
...
PROMPT Checking the partial installation before resuming
DECLARE
...
BEGIN
  IF USER <> 'AI_POD_STAFFING' THEN
    RAISE_APPLICATION_ERROR(-20021, 'Wrong schema. Connect as AI_POD_STAFFING. Current user: ' || USER);
  END IF;

  SELECT COUNT(*)
    INTO l_table_count
    FROM USER_TABLES
   WHERE TABLE_NAME IN ('PROJECT_TYPES', 'INTERESTS', 'PEOPLE', 'CUSTOMER_MAPPING', 'DELIVERABLES', 'DELIVERABLE_SKILLS', 'PERSON_INTERESTS', 'AVAILABILITY', 'REQUESTS', 'REQUIREMENTS', 'RECOMMENDATIONS', 'ELIGIBILITY_RULES', 'LOAD_GUARDRAILS', 'SCORING_WEIGHTS', 'AGENT_EXECUTIONS', 'APPROVAL_DECISIONS', 'POD_ASSIGNMENTS');

  IF l_table_count <> 17 THEN
    RAISE_APPLICATION_ERROR(-20022, 'Expected the 17 tables created before the failure; found ' || l_table_count);
  END IF;
  ...
  IF l_audit_count <> 0 THEN
    RAISE_APPLICATION_ERROR(-20023, 'AUDIT_EVENTS already exists. Resume stopped without changing data.');
  END IF;
  ...
  FOR r IN (
    ...
    IF l_row_count <> 0 THEN
      RAISE_APPLICATION_ERROR(-20024, r.TABLE_NAME || ' is not empty. Resume stopped to prevent duplicate data.');
    END IF;
  END LOOP;
END;
/

PROMPT Partial installation is safe to resume
PROMPT Creating corrected Oracle blockchain audit table
