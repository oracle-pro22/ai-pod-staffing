...
class DecisionStore:
    ...
    def decide(self, actor, decision):
        ...
        with self.database.write() as connection:
            ...
            review = load_review(connection, decision.proposal_id)
            selected_ids = {m.person_id for m in review[1].selected.proposal.members} if review else set()
            for pid in sorted(selected_ids | {member["person_id"] for member in members}):
            ...
            prior = rows(connection, "SELECT d.*,p.proposal_version,p.validation_json FROM approval_decisions d JOIN pod_proposals p ON p.proposal_id=d.proposal_id WHERE d.idempotency_key=:idemKey", idemKey=key)
            if prior:
                old = prior[0]
                provenance = document(old["validation_json"]) if old.get("validation_json") else {}
                links = rows(connection, "SELECT source_proposal_id,source_proposal_version,selection_id FROM mvp_p2_decisions WHERE decision_id=:decisionId", decisionId=old["decision_id"])
                if links:
                    provenance = links[0]
                if (provenance.get("source_proposal_id", old["proposal_id"]) != decision.proposal_id or old["action_type"] != decision.action
                    or provenance.get("source_proposal_version", old["proposal_version"]) != decision.proposal_version
                    or provenance.get("selection_id") != decision.selection_id
                    or (old["reason"] or "") != (decision.reason or "")):
                    raise ServiceError("IDEMPOTENCY_CONFLICT", "This decision key was already used for a different decision.", 409)
                return {"decision_id": old["decision_id"], "proposal_id": old["proposal_id"], "status": old["action_type"], "replayed": True}
            ...
            if pending_manual(connection, request_id, request['request_revision']):
                raise ServiceError('MANUAL_DRAFT_ACTIVE', 'Review or discard the saved manual draft before deciding the original recommendation.', 409)
            ...
            if decision.action == "APPROVED":
                ...
                for member in ([] if selected_review else members):
                ...
                pod = selected_review.selected.proposal if selected_review else Proposal(request_id=request_id, request_revision=request["request_revision"], policy_version=policy_version,
            ...
            elif review and decision.selection_id != (review[2]["selection_id"] if review[2] else None):
                raise ServiceError("STALE_SELECTION", "Review the latest saved selection before rejecting.", 409)
            if selected_review:
                # A Captain-selected snapshot is a new proposal revision derived
                # from a real execution, never an edited/fictitious AI result.
                from app.proposal_writer import write_proposal
                final_id, final_version = write_proposal(connection, fresh, selected_review.selected,
                    proposal["execution_id"], {"origin": "CAPTAIN_SELECTION", "source_proposal_id": decision.proposal_id,
                    "source_proposal_version": decision.proposal_version, "selection_id": decision.selection_id,
                    "selection_sources": selected_review.sources, "issues": [], "policy_version": policy_version},
                    selected_review.rationale)
                decision = decision.model_copy(update={"proposal_id": final_id, "proposal_version": final_version})
            return commit_final_decision(connection, actor, decision, proposal, request_id, policy_version,
                                         pod, fresh if pod else None, validation, review)


def commit_final_decision(connection, actor, decision, proposal, request_id, policy_version,
                          pod, fresh, validation, review=None, idempotency_key=None):
    """Shared atomic writer; callers have authorized, locked and validated already."""
    key = idempotency_key or hashlib.sha256(f"{actor.subject}|{decision.idempotency_key}".encode()).hexdigest()
    ...
    if review:
        execute(connection, """INSERT INTO mvp_p2_decisions(decision_id,source_proposal_id,source_proposal_version,selection_id)
            VALUES(:decisionId,:sourceId,:sourceVersion,:selectionId)""",
            {"decisionId": decision_id, "sourceId": proposal["proposal_id"],
             "sourceVersion": proposal["proposal_version"], "selectionId": decision.selection_id})
