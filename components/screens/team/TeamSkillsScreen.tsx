...
export function TeamSkillsScreen() {
  ...
  return <section className="staffing-screen">
    <PageHeader title="Team & skills" description="Customer-defined skills, evidence, strength, allocation, and current pod information." actions={<><TextField value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Find a team member or skill" />{state.role === 'Administrator' && canPerform(state.role, 'TEAM_SKILLS', 'canCreate', data.authorization) ? <Button variant="primary" onClick={() => dispatch({ type: 'open-modal', modal: { id: 'add-person', title: 'Add person' } })}>＋ Add person</Button> : null}</>} />
    {person ? <div className="staffing-profile-grid">
      <Card className="staffing-profile-card"><Avatar initials={person.initials} /><h3>{person.name}</h3><p>{person.jobTitle} • {person.location}</p><div className="staffing-stat-pairs"><div><strong>{person.allocationPct}%</strong><span>Current allocation</span></div><div><strong>{person.activePods}</strong><span>Active pods</span></div><div><strong>{average}</strong><span>Average strength</span></div><div><strong>{person.skills.length}</strong><span>Rated skills</span></div></div></Card>
