...
export function TeamSkillsScreen() {
  ...
  const people = useMemo(() => {
    const scoped = selectVisiblePeople(data, state.role, state.drafts).filter((item) => `${item.name} ${item.jobTitle} ${item.skills.map((skill) => skill.name).join(' ')}`.toLowerCase().includes(query.toLowerCase()));
  ...
  }, [data, query, sort, state.role, state.drafts]);
  ...
  return <section className="staffing-screen">
    <PageHeader title="Team & skills" description="Customer-defined skills, evidence, strength, allocation, and current pod information." actions={!isMember ? <><TextField value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Find a team member or skill" />{!isScopedRole(state.role) ? <Button variant="primary" onClick={() => notify('Draft action', 'The add-person flow would connect to enterprise identity in production.')}>＋ Add person</Button> : null}</> : undefined} />
    ...
    {!isMember ? <Card className="staffing-section-gap"><CardHeader><div><h3>Team directory</h3><p>{people.length} person{people.length === 1 ? '' : 's'} visible to {state.role}</p></div><SelectField value={sort} onChange={(event) => setSort(event.target.value)}><option value="availability">Sort by availability</option><option value="name">Sort by name</option><option value="skill">Sort by strongest skill</option></SelectField></CardHeader><div className="staffing-list">{people.map((item) => <div className="staffing-list-row" key={item.id}><div className="staffing-person-line"><Avatar initials={item.initials} /><span><b>{item.name}</b><small>{item.jobTitle} • {item.location}</small></span></div><div className="staffing-tag-row">{item.skills.slice(0, 2).map((skill) => <span key={skill.id}>{skill.name}</span>)}</div><div><b>{item.allocationPct}%</b><small>{item.activePods} pods</small></div><Button size="small" onClick={() => dispatch({ type: 'open-drawer', drawer: { id: 'person-details', title: item.name, payload: { personId: item.id } } })}>View</Button></div>)}</div></Card> : null}
  </section>;
}
