...
import { canPerform, hasGrantedRole } from '@/lib/role-policy';
...
export function LiveAssignments({ view = 'projects' }: { view?: 'projects' | 'calendar' | 'reports' }) {
  ...
  async function closeProject() {
    ...
    if (!request || !request.can_close || !reason.trim() || busy) return;
  ...
  return <section className="staffing-screen">
    <PageHeader title={title} description={view === 'projects' ? 'Review requests, final POD assignments, and project responsibilities.' : 'Confirmed assignments and current weekly capacity.'}
      actions={<><Button disabled={busy} onClick={() => { setError(''); refresh(); }}>Refresh</Button>
        ...
        {view === 'projects' && canPerform(state.role, 'REQUESTS', 'canCreate', data.authorization) && <Button variant="primary" onClick={() => dispatch({ type: 'open-modal', modal: { id: 'create-request', title: 'Create staffing request' } })}>＋ New request</Button>}</>} />
    ...
    {snapshot && view !== 'calendar' && snapshot.requests.map(request => {
      ...
      return <Card key={request.request_id}><CardHeader><div><h3>{request.title}</h3><p>{request.request_id} · {request.status.replaceAll('_', ' ')}</p></div>
        {view === 'projects' && hasGrantedRole('POD Captain', data.authorization) && canPerform(state.role, 'AI_FITMENT', 'canView', data.authorization) && <Button onClick={() => { dispatch({ type: 'set-active-request', requestId: request.request_id }); dispatch({ type: 'set-screen', screen: 'fitment' }); }}>Review fitment</Button>}
        {request.status === 'STAFFED' && request.can_close === true && <Button disabled={busy} onClick={() => { setClosing(request.request_id); setReason(''); }}>Close project</Button>}
      </CardHeader><CardBody>
        ...
        {closing === request.request_id && request.status === 'STAFFED' && request.can_close === true && <div style={{ marginTop: 20 }}><p>{CLOSURE_EXPLANATION}</p><small>Dates follow the assignment’s staffing-policy timezone. This does not record actual hours worked.</small>
