...
function PreviewCalendar() {
  ...
  return <section className="staffing-screen">
    <PageHeader title="Allocation calendar" description="Weekly capacity planning with leave, travel, active pods, and overload guardrails." actions={<><Button onClick={() => dispatch({ type: 'set-week-offset', weekOffset: state.weekOffset - 1 })}>←</Button><Button onClick={() => dispatch({ type: 'set-week-offset', weekOffset: 0 })}>This week</Button><Button onClick={() => dispatch({ type: 'set-week-offset', weekOffset: state.weekOffset + 1 })}>→</Button>{canPerform(state.role, 'ALLOCATION_CALENDAR', 'canCreate', data.authorization) ? <Button variant="primary" onClick={() => dispatch({ type: 'open-drawer', drawer: { id: 'quick-allocation', title: 'Quick allocation' } })}>＋ Allocate</Button> : null}</>} />
